...
export class KugoApi {
    ...
    async search(search, page = 1, pageSize = 10) {
        try {
    ...
    async getSongInfo(albumId, albumAudioId, hash) {
        const params = {
            appid: this.appid,
            clienttime: Date.now(),
            clientver: 20000,
     ...
     */
    async getSign(params, ispost = false) {
    ...
    async getDfid() {
        ...
        if (!dfid)
            throw new Error(`获取dfid失败${JSON.stringify(data)}`);
        return dfid;
    }
    async getLoginQrcode() {
        ...
        // eslint-disable-next-line camelcase
        if (!qrcode || !qrcode_img)
            throw new Error(`获取登录二维码失败\n${JSON.stringify(data)}`);
        // eslint-disable-next-line camelcase
        return { qrcode, qrcode_img };
    }
    async getUserInfoQrcode(qrcode) {
        const params = {
            appid: this.appid,
            clientver: 8131,
            clienttime: new Date().getTime(),
    ...
    /** 通过token获取登录ck */
    async getLoginByToken(token = this.token, userid = this.userid) {
        const now = Date.now();
